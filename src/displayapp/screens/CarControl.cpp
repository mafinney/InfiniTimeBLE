...
CarControl::CarControl(Pinetime::Controllers::ESPService& espService, Pinetime::System::SystemTask& systemTask) : espService {espService}, systemTask {systemTask} {
    ...
    incr.button = lv_btn_create(car_screen, nullptr);
    incr.button->user_data = this;
    lv_obj_set_event_cb(incr.button, ButtonEvent);
    lv_obj_set_size(incr.button, SMALL_BUTTON_WIDTH, SMALL_BUTTON_HEIGHT);
    lv_obj_align(incr.button, car_screen, LV_ALIGN_IN_BOTTOM_LEFT, 0, 0);
    incr.label = lv_label_create(incr.button, nullptr);
    lv_label_set_text_static(incr.label, (char *) "Incr");
    
    CreateLabel(&count_lbl, car_screen, SMALL_BUTTON_WIDTH, SMALL_BUTTON_HEIGHT, LV_ALIGN_IN_TOP_LEFT, 0, 0, (char *) "lbl");

    send.button = lv_btn_create(car_screen, nullptr);
    send.button->user_data = this;
    lv_obj_set_event_cb(send.button, ButtonEvent);
    lv_obj_set_size(send.button, SMALL_BUTTON_WIDTH, SMALL_BUTTON_HEIGHT);
    lv_obj_align(send.button, car_screen, LV_ALIGN_IN_BOTTOM_RIGHT, 0, 0);
    send.label = lv_label_create(send.button, nullptr);
    lv_label_set_text_static(send.label, (char *) "Send");
